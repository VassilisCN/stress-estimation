...
for i, valid_subject in enumerate(subjects):  # Use 1 subject for validation.
    ...
    train_stress_dataset = STRESS_dataset(subjects=train_subjects, max_frame=3286, multiclass=True)
    valid_stress_dataset = STRESS_dataset(subjects=valid_subjects, max_frame=3286, multiclass=True)
    ...
    model = StressDetectionModel(input_dim=train_stress_dataset[0][0].shape[1], embed_dim=128, num_classes=7).to(device)
    ...
    for epoch in range(epochs):
        ...
        for batch_x, batch_y in train_dl:  # batch_x: [B, T, F], batch_y: [B]
            ...
            # one_hot = torch.zeros((batch_y.size(0), 2), dtype=torch.long, device=device)
            # one_hot[torch.arange(batch_y.size(0)), batch_y] = 1
            # loss = criterion(logits, one_hot.float())
    ...
    print(f'Subject [{i+1}/{len(subjects)}], Validation Loss: {epoch_validation_loss:.4f}, Best Validation Loss: {min(validation_losses):.4f}, Validation Accuracy: {epoch_validation_accuracy:.4f}, Best Validation Accuracy: {max(validation_accuracies):.4f}')
